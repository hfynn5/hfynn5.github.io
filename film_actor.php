...
<br>
  ...
  <?php require_once 'processfilmactor.php'; ?>
  <?php
  ...
  <form action="processfilmactor.php" method="POST">
    <input type="hidden" name="id" value="<?php echo $ID; ?>" readonly="readonly">
      ...
      <div class="form-group">
        <label>Film ID</label>
        <input type="text" name="filmID" class="form-control" value="<?php echo $filmID ?>" placeholder="Enter Film ID">
      </div>
      <div class="form-group">
        <?php
        if($update == true):
  ...
  <?php
        while($row = $result->fetch_assoc()): ?>
          <tr>
              ...
              <td><center><a href="film_actor.php?edit=<?php echo $row['actor_id']; ?>" class="btn btn-info">Edit</a>
              <a href="film_actor.php?delete=<?php echo $row['actor_id']; ?>" class="btn btn-danger">Delete</a></center></td>
          </tr>
        <?php endwhile; ?>
      </table>
  </div>
